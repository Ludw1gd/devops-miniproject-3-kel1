...
# FIX #1 — Definisi VM menggunakan map for_each
# Setiap VM punya identitas berbeda: peran, ukuran, dan aturan port sendiri
variable "vms" {
  description = "Definisi setiap VM: peran, ukuran, dan apakah expose port 8080"
  type = map(object({
    role      = string  # Label peran VM (digunakan sebagai tag)
    size      = string  # SKU Azure VM
    open_8080 = bool    # true = buka port 8080 di NSG (khusus Jenkins)
  }))
  default = {
    jenkins = {
      role      = "jenkins-node"
      size      = "Standard_B2s"  # 2vCPU, 4GB RAM — cukup untuk Jenkins master
      open_8080 = true            # Jenkins UI & API berjalan di port 8080
    }
    target = {
      role      = "target-node"
      size      = "Standard_B1s"  # 1vCPU, 1GB RAM — cukup untuk build agent
      open_8080 = false           # Target node TIDAK perlu expose 8080 ke publik
    }
  }
}
...
# FIX #3 — Output public key untuk referensi / debugging
# Private key TIDAK di-output ke console (sudah disimpan di file .pem)
